# Computer Graphics PA2: Drawing Basis Function.
import sys
import argparse
import numpy as np
...
class Bernstein:
    ...
    def __init__(self, n, k, t: np.ndarray):
        ...
        self.k = k
        self.t = t
        self.tpad = np.pad(self.t, (0, self.k), 'constant', constant_values=(0., self.t[-1]))
        ...
        for p in range(1, self.k + 1):
            for ii in range(self.k - p, self.k + 1):
                ...
                if self.tpad[i + p] == self.tpad[i]:
                    ...
                    w1 = (mu - self.tpad[i]) / (self.tpad[i + p] - self.tpad[i])
                    dw1 = 1 / (self.tpad[i + p] - self.tpad[i])
                if self.tpad[i + p + 1] == self.tpad[i + 1]:
                    w2 = 1 - mu
                    dw2 = -1.
                else:
                    w2 = (self.tpad[i + p + 1] - mu) / (self.tpad[i + p + 1] - self.tpad[i + 1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bspline', action='store_true', help='Draw B-Spline instead of Bezier')
    parser.add_argument('--valid', action='store_true', help='Draw only valid range of parameter')
    ...
    if args.valid:
        t_range = np.linspace(b.get_valid_range()[0], b.get_valid_range()[1], 200)
    else:
